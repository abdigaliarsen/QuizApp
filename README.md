...
I used bootstrap to easily design pages. I used React Bootstrap and the regular one and even mixed them:
...
The QuizApp uses [MSSQL Server](https://en.wikipedia.org/wiki/Microsoft_SQL_Server). The `QuizAppDatabase` stores data about users, quizzes, questions, answers, and db migrations. Since I used `EntityFrameworkCore` to manage the database access, and `IdentityCore` to easily handle user data, there are bunch of AspNetXXX tables that are generated by EFCore.
...
When users successfuly logged in, they receive generated `JWT` token that is used to verify the user in future `API` calls. The generated `JWT` token consists of payload that allows to get user data in server after its descryption with project's secret key that is stored in [appsettings.json](https://github.com/quadropunk/QuizApp/blob/main/QuizApp.WebAPI/appsettings.json).
...
This token is used in any user functions, such as: `setCompletedQuizToCurrentUser`, `isQuizCompletedByCurrentUser`, `createQuiz` in [api.jsx](https://github.com/quadropunk/QuizApp/blob/main/QuizApp.WebUI/src/components/api.jsx).
...
Each of the controller has corresponding readonly service interface to access Business Layer data manipulation results. These interfaces are connected with their implementation classes in [`Program.cs`](https://github.com/quadropunk/QuizApp/blob/main/QuizApp.WebAPI/Program.cs) file, like:

```C#
builder.Services.AddScoped<IAnswerService, AnswerService>();
```

So the `AnswerService` functions will be used anytime the program will call `IAnswerSerive` interface.
